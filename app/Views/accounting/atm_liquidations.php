...
<div class="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-6">
    <div>
        ...
        <p class="text-gray-600 mt-2 text-sm">ATM liquidation records approved by chairman and ready for accounting processing</p>
    </div>
    ...
    foreach ($batchRecords as $record) {
        ...
        if (!isset($batches[$batchId])) {
            $batches[$batchId] = [
                ...
                'chairman_approval_date' => $record['approved_at'] ?? null,
                'recipient_count' => 0,
                'total_amount' => 0,
                'total_records' => 0
            ];
    ...
    <div class="flex items-center space-x-4">
        <span class="bg-green-100 text-green-800 px-4 py-2 rounded-lg font-semibold">
            <?= count($individualRecords) + count($batches) ?> Approved
        </span>
                    ...
                    <tr class="hover:bg-green-50/20 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                ...
                                <button type="button" 
                                        onclick="receiveModal(<?= $liquidation['id'] ?>, '<?= esc($liquidation['recipient_name']) ?>', false)"
            ...
            <table class="min-w-full divide-y divide-gray-200 data-table-batch">
                <thead class="bg-gradient-to-r from-blue-600 to-blue-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Status</th>
                        <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Approved</th>
                        <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50/20 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <?php if (!empty($batch['chairman_approval_date'])): ?>
                                <div class="text-sm text-gray-900"><?= date('M j, Y', strtotime($batch['chairman_approval_date'])) ?></div>
                                <div class="text-xs text-gray-500"><?= date('g:i A', strtotime($batch['chairman_approval_date'])) ?></div>
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                ...
                                <button type="button" 
                                        onclick="receiveModal(<?= $batch['id'] ?>, '<?= esc($batch['batch_name']) ?>', true)"
                                        class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-full hover:bg-green-200 transition-colors"
                                        title="Receive Batch">
                                    <i class="bi bi-inbox text-sm"></i>
                                </button>
...
<div id="receiveModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full">
        <div class="p-6">
            ...
            <div class="mb-4">
                <p class="text-sm text-gray-700"><span id="receiveLabel">Batch:</span> <span id="receiveBatchName" class="font-medium"></span></p>
            </div>
...
function receiveModal(id, name, isBatchRecord) {
    currentLiquidationId = id;
    isBatch = isBatchRecord;
    document.getElementById('receiveLabel').textContent = isBatch ? 'Batch:' : 'Recipient:';
    document.getElementById('receiveBatchName').textContent = name;
...
function confirmReceive() {
    ...
    const url = isBatch ? '<?= base_url('accounting/receive-atm-batch') ?>' : '<?= base_url('accounting/receive-atm') ?>';
